...
def prompt_engineering_demo(user_input, system_prompt, temperature):
    ...
    for prompt_type, prompt in prompts.items():
        ...
        if prompt_type == "专家":
            response = (
                f"作为{prompt_type}，我的专业分析是：{user_input}涉及复杂的专业概念..."
            )
        elif prompt_type == "教师":
...
with gr.Blocks(title="LLM 应用开发学习 Demo", theme=gr.themes.Soft()) as demo:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                max_tokens = gr.Slider(
                    100, 2000, value=1000, step=100, label="Max Tokens"
                )

        submit.click(
            chat_with_llm, [msg, chatbot, temperature, max_tokens], [msg, chatbot]
        )
        msg.submit(
            chat_with_llm, [msg, chatbot, temperature, max_tokens], [msg, chatbot]
        )
        ...
        with gr.Row():
            with gr.Column():
                text_input = gr.Textbox(
                    label="输入文本", placeholder="请输入要总结的文本...", lines=8
                )
        ...
        with gr.Row():
            with gr.Column():
                code_input = gr.Textbox(
                    label="输入代码", placeholder="请输入要解释的代码...", lines=10
                )
    ...
    with gr.Tab("🎯 提示工程"):
        ...
        with gr.Row():
            with gr.Column():
                prompt_input = gr.Textbox(
                    label="用户输入", placeholder="请输入您的问题...", lines=3
                )
                system_prompt = gr.Textbox(
                    label="系统提示", value="你是一个有用的AI助手。", lines=2
                )
        ...
        prompt_btn.click(
            ...
            [prompt_input, system_prompt, prompt_temperature],
            prompt_output,
        )
        ...
        with gr.Row():
            with gr.Column():
                query_input = gr.Textbox(
                    label="查询问题", placeholder="请输入您的问题...", lines=2
                )
