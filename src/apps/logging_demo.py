...
class LoggingDemoApp(GradioApp):
    ...
    def __init__(self):
        super().__init__(
            name="logging_demo", description="Python 日志模块演示", port=7862
        )
        self.setup_logging()
    ...
    def demo_error_logging(self, error_type, error_message):
        ...
        try:
            ...
            # 记录错误日志
            self.logger.error(
                f"捕获到错误: {type(e).__name__}: {str(e)}", exc_info=True
            )
            return (
        ...
        try:
            ...
            for file in os.listdir(log_dir):
                if file.endswith(".log"):
                    ...
                    log_files.append(
                        f"📄 {file} ({size} bytes, 修改时间: {modified.strftime('%Y-%m-%d %H:%M:%S')})"
                    )
        ...
        with gr.Blocks(title="Python 日志模块演示") as interface:
            ...
            with gr.Tabs():
                    ...
                    with gr.Row():
                        ...
                        with gr.Column():
                            basic_output = gr.Textbox(
                                label="结果", lines=8, interactive=False
                            )
                ...
                with gr.TabItem("🏗️ 结构化日志"):
                    ...
                    with gr.Row():
                        with gr.Column():
                            ...
                            details = gr.Textbox(
                                label="详细信息", placeholder="用户登录系统", lines=2
                            )
                            structured_btn = gr.Button(
                                "记录结构化日志", variant="primary"
                            )
                    ...
                    with gr.Row():
                        ...
                        with gr.Column():
                            perf_output = gr.Textbox(
                                label="结果", lines=6, interactive=False
                            )
                ...
                with gr.TabItem("🔍 日志级别测试"):
                    ...
                    with gr.Row():
                        ...
                        level_output = gr.Textbox(
                            label="结果", lines=10, interactive=False
                        )

                    level_btn.click(
                        fn=self.demo_log_levels, inputs=[], outputs=[level_output]
                    )
                ...
                with gr.TabItem("📁 日志管理"):
                    ...
                    list_btn.click(
                        fn=self.get_log_files, inputs=[], outputs=[manage_output]
                    )

                    clear_btn.click(
                        fn=self.clear_logs, inputs=[], outputs=[manage_output]
                    )
