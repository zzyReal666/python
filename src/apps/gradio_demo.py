...
with gr.Blocks(title="Gradio 多功能演示", theme=gr.themes.Soft()) as demo:
    ...
    with gr.Tab("👋 问候"):
        ...
        with gr.Row():
            with gr.Column():
                ...
                temperature = gr.Slider(minimum=-50, maximum=50, value=20, label="温度 (°C)")
                greet_btn = gr.Button("生成问候", variant="primary")
            with gr.Column():
                greet_output = gr.Textbox(label="问候结果", lines=3)
        
        greet_btn.click(greet, inputs=[name_input, is_morning, temperature], outputs=greet_output)
    ...
    with gr.Tab("📈 图表生成"):
        ...
        chart_btn.click(generate_chart, inputs=[chart_type, data_points], outputs=chart_output)
    ...
    with gr.Tab("😊 情感分析"):
        ...
        sentiment_btn.click(sentiment_analyzer, inputs=sentiment_text, outputs=sentiment_output)
